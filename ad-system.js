// ===============================
// نظام الروابط الإعلانية
// ===============================

// --- قائمة الروابط الإعلانية (50+ رابط) ---
const adLinks = [
    "https://www.effectivegatecpm.com/f24em5wax?key=ee9cb9ab641e0b2bc42940c78a430941",
    "https://www.effectivegatecpm.com/f7pxzt00y?key=846c88991d57c3a0d04b02a4ba515188",
    "https://www.effectivegatecpm.com/f24em5wax?key=ee9cb9ab641e0b2bc42940c78a430941",
    "https://www.effectivegatecpm.com/f24em5wax?key=ee9cb9ab641e0b2bc42940c78a430941",
    "https://www.effectivegatecpm.com/f24em5wax?key=ee9cb9ab641e0b2bc42940c78a430941",
    "https://www.effectivegatecpm.com/f24em5wax?key=ee9cb9ab641e0b2bc42940c78a430941",
    "https://www.effectivegatecpm.com/f24em5wax?key=ee9cb9ab641e0b2bc42940c78a430941",
    "https://www.effectivegatecpm.com/f24em5wax?key=ee9cb9ab641e0b2bc42940c78a430941",
    "https://www.effectivegatecpm.com/f24em5wax?key=ee9cb9ab641e0b2bc42940c78a430941",
    "https://www.effectivegatecpm.com/f24em5wax?key=ee9cb9ab641e0b2bc42940c78a430941",
    "https://www.effectivegatecpm.com/f24em5wax?key=ee9cb9ab641e0b2bc42940c78a430941",
    "https://www.effectivegatecpm.com/f24em5wax?key=ee9cb9ab641e0b2bc42940c78a430941",
    "https://www.effectivegatecpm.com/f24em5wax?key=ee9cb9ab641e0b2bc42940c78a430941",
    "https://www.effectivegatecpm.com/f24em5wax?key=ee9cb9ab641e0b2bc42940c78a430941",
    "https://www.effectivegatecpm.com/f24em5wax?key=ee9cb9ab641e0b2bc42940c78a430941",
    "https://www.effectivegatecpm.com/f24em5wax?key=ee9cb9ab641e0b2bc42940c78a430941",
    "https://www.effectivegatecpm.com/f24em5wax?key=ee9cb9ab641e0b2bc42940c78a430941",
    "https://www.effectivegatecpm.com/f24em5wax?key=ee9cb9ab641e0b2bc42940c78a430941",
    "https://www.effectivegatecpm.com/f24em5wax?key=ee9cb9ab641e0b2bc42940c78a430941",
    "https://www.effectivegatecpm.com/f24em5wax?key=ee9cb9ab641e0b2bc42940c78a430941",
    "https://www.effectivegatecpm.com/f24em5wax?key=ee9cb9ab641e0b2bc42940c78a430941",
    "https://www.effectivegatecpm.com/f24em5wax?key=ee9cb9ab641e0b2bc42940c78a430941",
    "https://www.effectivegatecpm.com/f24em5wax?key=ee9cb9ab641e0b2bc42940c78a430941",
    "https://www.effectivegatecpm.com/f24em5wax?key=ee9cb9ab641e0b2bc42940c78a430941",
    "https://www.effectivegatecpm.com/f24em5wax?key=ee9cb9ab641e0b2bc42940c78a430941",
    "https://www.effectivegatecpm.com/f24em5wax?key=ee9cb9ab641e0b2bc42940c78a430941",
    "https://www.effectivegatecpm.com/f24em5wax?key=ee9cb9ab641e0b2bc42940c78a430941",
    "https://www.effectivegatecpm.com/f24em5wax?key=ee9cb9ab641e0b2bc42940c78a430941",
    "https://www.effectivegatecpm.com/f24em5wax?key=ee9cb9ab641e0b2bc42940c78a430941",
    "https://www.effectivegatecpm.com/f24em5wax?key=ee9cb9ab641e0b2bc42940c78a430941",
    "https://www.effectivegatecpm.com/f24em5wax?key=ee9cb9ab641e0b2bc42940c78a430941",
    "https://www.effectivegatecpm.com/f24em5wax?key=ee9cb9ab641e0b2bc42940c78a430941",
    "https://www.effectivegatecpm.com/f24em5wax?key=ee9cb9ab641e0b2bc42940c78a430941",
    "https://www.effectivegatecpm.com/f24em5wax?key=ee9cb9ab641e0b2bc42940c78a430941",
    "https://www.effectivegatecpm.com/f24em5wax?key=ee9cb9ab641e0b2bc42940c78a430941",
    "https://www.effectivegatecpm.com/f24em5wax?key=ee9cb9ab641e0b2bc42940c78a430941",
    "https://www.effectivegatecpm.com/f24em5wax?key=ee9cb9ab641e0b2bc42940c78a430941",
    "https://www.effectivegatecpm.com/f24em5wax?key=ee9cb9ab641e0b2bc42940c78a430941",
    "https://www.effectivegatecpm.com/f24em5wax?key=ee9cb9ab641e0b2bc42940c78a430941",
    "https://www.effectivegatecpm.com/f24em5wax?key=ee9cb9ab641e0b2bc42940c78a430941",
    "https://www.effectivegatecpm.com/f24em5wax?key=ee9cb9ab641e0b2bc42940c78a430941",
    "https://www.effectivegatecpm.com/f24em5wax?key=ee9cb9ab641e0b2bc42940c78a430941",
    "https://www.effectivegatecpm.com/f24em5wax?key=ee9cb9ab641e0b2bc42940c78a430941",
    "https://www.effectivegatecpm.com/f24em5wax?key=ee9cb9ab641e0b2bc42940c78a430941",
    "https://www.effectivegatecpm.com/f24em5wax?key=ee9cb9ab641e0b2bc42940c78a430941",
    "https://www.effectivegatecpm.com/f24em5wax?key=ee9cb9ab641e0b2bc42940c78a430941",
    "https://www.effectivegatecpm.com/f24em5wax?key=ee9cb9ab641e0b2bc42940c78a430941",
    "https://www.effectivegatecpm.com/f24em5wax?key=ee9cb9ab641e0b2bc42940c78a430941",
    "https://www.effectivegatecpm.com/f24em5wax?key=ee9cb9ab641e0b2bc42940c78a430941",
    "https://www.effectivegatecpm.com/f24em5wax?key=ee9cb9ab641e0b2bc42940c78a430941"
];

// --- رسائل النافذة المنبثقة (20 رسالة) ---
const popupMessages = [
    "هذه الإعلانات هي مصدر دعم الموقع ❤️",
    "بفضل الإعلانات يبقى الموقع مجاني دائمًا 🌟",
    "شكرًا لتفهمك ودعمك المستمر 🙏",
    "كل إعلان يساعدنا في تحسين الخدمة ✨",
    "نحن نقدر وقتك ودعمك لنا 💙",
    "الإعلانات تساعدنا في تقديم محتوى أفضل 🎬",
    "شكرًا لك! أنت تساهم في بقاء الموقع 🌸",
    "إعلان واحد = دعم كبير للموقع 💪",
    "تفهمك يعني لنا الكثير، شكرًا 🌺",
    "معًا نحافظ على الموقع مجانيًا 🤝",
    "دعمك من خلال الإعلانات يهمنا 💖",
    "نشكرك على صبرك ودعمك 🎭",
    "الإعلانات تبقي المحتوى متاحًا للجميع 🌍",
    "أنت جزء من نجاح الموقع، شكرًا 🏆",
    "كل مشاهدة إعلان = خدمة أفضل لك 🎯",
    "شكرًا لمساهمتك في استمرارية الموقع 🌈",
    "بدعمك نستمر في تقديم الأفضل 🎪",
    "الإعلانات قصيرة، لكن فائدتها كبيرة 💫",
    "نقدّر تعاونك ودعمك لموقعنا 🎨",
    "معًا نصنع تجربة مشاهدة رائعة 🎉"
];

// --- متغيرات النظام ---
let currentAdIndex = 0; // العداد الحالي للإعلانات
let lastAdTime = 0; // آخر وقت تم فتح إعلان فيه
const AD_COOLDOWN = 60000; // مدة الانتظار بين الإعلانات (1 دقيقة = 60000 ميلي ثانية)

// --- حفظ واستعادة حالة النظام ---
function saveAdSystemState() {
    localStorage.setItem('currentAdIndex', currentAdIndex);
    localStorage.setItem('lastAdTime', lastAdTime);
}

function loadAdSystemState() {
    const savedIndex = localStorage.getItem('currentAdIndex');
    const savedTime = localStorage.getItem('lastAdTime');
    
    if (savedIndex !== null) {
        currentAdIndex = parseInt(savedIndex);
    }
    
    if (savedTime !== null) {
        lastAdTime = parseInt(savedTime);
    }
}

// --- التحقق من إمكانية عرض الإعلان ---
function canShowAd() {
    const currentTime = Date.now();
    const timeSinceLastAd = currentTime - lastAdTime;
    
    // إذا مرت دقيقة واحدة منذ آخر إعلان، اسمح بعرض إعلان جديد
    return timeSinceLastAd >= AD_COOLDOWN || lastAdTime === 0;
}

// --- فتح رابط الإعلان ---
function openAdLink() {
    if (!canShowAd()) {
        console.log('يجب الانتظار دقيقة واحدة قبل عرض إعلان آخر');
        return false;
    }
    
    // الحصول على الرابط الحالي
    const adUrl = adLinks[currentAdIndex % adLinks.length];
    
    // فتح الرابط في صفحة جديدة
    window.open(adUrl, '_blank');
    
    // تحديث العداد والوقت
    currentAdIndex++;
    lastAdTime = Date.now();
    
    // حفظ الحالة
    saveAdSystemState();
    
    // إظهار النافذة المنبثقة بعد ثانيتين
    setTimeout(showPopupMessage, 2000);
    
    return true;
}

// --- إظهار النافذة المنبثقة ---
function showPopupMessage() {
    // اختيار رسالة عشوائية
    const randomIndex = Math.floor(Math.random() * popupMessages.length);
    const message = popupMessages[randomIndex];
    
    // إنشاء النافذة المنبثقة
    const popup = document.createElement('div');
    popup.className = 'ad-popup-overlay';
    popup.innerHTML = `
        <div class="ad-popup-content">
            <div class="ad-popup-icon">
                <i class="fas fa-heart"></i>
            </div>
            <h3 class="ad-popup-title">شكرًا لدعمك</h3>
            <p class="ad-popup-message">${message}</p>
            <button class="ad-popup-close" onclick="closeAdPopup()">
                <i class="fas fa-check"></i>
                حسنًا، فهمت
            </button>
        </div>
    `;
    
    document.body.appendChild(popup);
    
    // إضافة تأثير الظهور
    setTimeout(() => {
        popup.classList.add('active');
    }, 10);
}

// --- إغلاق النافذة المنبثقة ---
function closeAdPopup() {
    const popup = document.querySelector('.ad-popup-overlay');
    if (popup) {
        popup.classList.remove('active');
        setTimeout(() => {
            popup.remove();
        }, 400);
    }
}

// --- دالة مساعدة: تنفيذ إعلان قبل وظيفة ---
function executeWithAd(originalFunction, ...args) {
    if (canShowAd()) {
        // فتح الإعلان أولاً
        openAdLink();
        
        // تنفيذ الوظيفة الأصلية بعد ثانية واحدة
        setTimeout(() => {
            originalFunction(...args);
        }, 1000);
    } else {
        // تنفيذ الوظيفة مباشرة بدون إعلان
        originalFunction(...args);
    }
}

// --- تهيئة نظام الإعلانات ---
function initAdSystem() {
    loadAdSystemState();
    console.log('✅ تم تهيئة نظام الإعلانات');
}

// --- تصدير الدوال للاستخدام العام ---
window.adSystem = {
    canShowAd,
    openAdLink,
    executeWithAd,
    showPopupMessage,
    closeAdPopup
};

// --- تهيئة النظام عند تحميل الصفحة ---
document.addEventListener('DOMContentLoaded', initAdSystem);
